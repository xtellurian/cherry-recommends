...
sqlcmd -S $SERVER.database.windows.net -U $SQLSERVERUSERNAME -P $SQLSERVERPASSWORD -d "master" -v AdminUserName=$ADMINUSERNAME AdminPassword=$ADMINPASSWORD ReadUserName=$READUSERNAME ReadPassword=$READPASSWORD -i create-db-principal.sql
