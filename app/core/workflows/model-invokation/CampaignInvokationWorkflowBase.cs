...
{
    public abstract class CampaignInvokationWorkflowBase<T> where T : CampaignEntityBase
    {
        private readonly ICampaignStore<T> store;
...
#nullable enable
        public async Task<Paginated<InvokationLogEntry>> QueryInvokationLogs(IPaginate paginate, CampaignEntityBase recommender)
        {
        ...
        {
            var isDisabled = await IsCampaignDisabled(recommender);
            if (isDisabled)
            {
                throw new RecommenderInvokationException($"Campaign {recommender.Id} Disabled or Expired", "The campaign settings are preventing invokation.");
            }
        }

        public async Task<bool> IsCampaignDisabled(T recommender)
        {
        ...
        /// </summary>
        protected void CheckArgument(CampaignEntityBase recommender,
                                    ...
                                    RecommendingContext context)
        {
            ...
            if (!input.Arguments.ContainsKey(arg.CommonId))
            {
                ...
                {
                    throw new BadRequestException("Missing campaign argument",
                        $"The argument {arg.CommonId} is required, and the campaign is set to throw on errors.");
                }
                else
                {
            ...
            else
            {
                ...
                {
                    throw new BadRequestException("Null campaign argument",
                        $"The argument {arg.CommonId} is null, and the campaign is set to throw on errors.");
                }
                else if (arg.ArgumentType == ArgumentTypes.Numerical)
                {
                    ...
                    if (!double.TryParse(val, out _))
                    {
                        ...
                        {
                            context.InvokationLog.LogMessage($"The argument {arg.CommonId} should be numeric, and the campaign is set to throw on errors.");
                            throw new BadRequestException("Bad campaign argument",
                                $"The argument {arg.CommonId} should be numeric, and the campaign is set to throw on errors.");
                        }
                        else
                        {
