...
namespace SignalBox.Core.Workflows
{
    public class RecommenderTriggersWorkflows : IRecommenderMetricTriggersWorkflow, IWorkflow
    {
        ...
        private async Task<IEnumerable<ItemsRecommendation>> HandleNewMetricValueItemsRecommenders(HistoricCustomerMetric metricValue)
        {
            ...
            {
                var isDisabled = await itemsRecommenderInvokationWorkflows.IsRecommenderDisabled(recommender);
                if (isDisabled)
                {
        ...
        private async Task<IEnumerable<ParameterSetRecommendation>> HandleNewMetricValueParameterSetRecommenders(HistoricCustomerMetric metricValue)
        {
            ...
            {
                var isDisabled = await parameterSetRecommenderInvokationWorkflows.IsRecommenderDisabled(recommender);
                if (isDisabled)
                {
                    logger.LogInformation($"Skipping trigger for recommender {recommender.Id}. Recommender is disabled");
                    continue;
