...
class TestPopulations(unittest.TestCase):
    ...
    def test_can_load_test_distribution(self):
        loaded = populations.PopulationDistributionCollection(
            deserialized_dict=json.loads(test_serialized_distribution()))
    ...
    def test_serialize_deserialize_population(self):
        population = populations.PopulationItemDistribution(
            str(uuid.uuid1()), test_items[0], items=test_items[0:2])
    ...
    def test_serialize_deserialize_population_collection(self):
        pop1 = populations.PopulationItemDistribution(
            str(uuid.uuid1()), default_item=test_items[0], items=test_items[0:2])
        pop1.add_items(test_items[1:2])
        pop2 = populations.PopulationItemDistribution(
            str(uuid.uuid1()), test_items[1], items=test_items[0:2])
        pop2.add_items(test_items[3:4])
    ...
    def test_population_probability_sum_one(self):
        ...
        population = populations.PopulationItemDistribution(
            str(uuid.uuid1()), default_item=test_items[0], items=test_items[0:3])
        ...
        chosen_items = population.choose_items(
            items=test_items[4:5], n_items=1)
