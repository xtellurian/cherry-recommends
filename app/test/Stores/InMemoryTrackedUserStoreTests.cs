...
namespace SignalBox.Test.Stores
{
    public class InMemoryTrackedUserStoreTests
    {
        ...
        public async Task CanStore_Empty_TrackedUserEvents()
        {
            ...
            var trackedEvent = new TrackedUserEvent(user, Guid.NewGuid().ToString(), updatedTime, null, EventKinds.Custom, "common_event_type", new DynamicPropertyDictionary
            {
        ...
        public async Task CanStore_TrackedUserEvents()
        {
            ...
            var event1 = new TrackedUserEvent(user, Guid.NewGuid().ToString(), updatedTime, null, EventKinds.Custom, "common_event_type", new DynamicPropertyDictionary
            {
                 {"a key", "a logical value"}
            });
            var event2 = new TrackedUserEvent(user, Guid.NewGuid().ToString(), updatedTime, null, EventKinds.Custom, "common_event_type", new DynamicPropertyDictionary
            {
                 {"a key", "a logical value"}
            });
        ...
        public async Task CanFilterOnGet_TrackedUserEvents()
        {
            ...
            var maryProperties = new TrackedUserEvent(maryTracked, Guid.NewGuid().ToString(), updatedTime, null, EventKinds.Custom, "Set_Name", new DynamicPropertyDictionary
            {
                {"name", "Mary"}
            });

            var garyProperties = new TrackedUserEvent(maryTracked, Guid.NewGuid().ToString(), updatedTime, null, EventKinds.Custom, "Set_Name", new DynamicPropertyDictionary
            {
                {"name", "Gary"}
            });
