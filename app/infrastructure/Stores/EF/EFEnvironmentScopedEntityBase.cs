...
namespace SignalBox.Infrastructure.EntityFramework
{
    public abstract class EFEnvironmentScopedEntityBase<T> : EFEntityStoreBase<T> where T : EnvironmentScopedEntity
    {
        ...
        protected EFEnvironmentScopedEntityBase(SignalBoxDbContext context, IEnvironmentService environmentService, Func<SignalBoxDbContext, DbSet<T>> selector)
        : base(context, selector)
        {
            this.environmentService = environmentService;
        }

        protected IEnvironmentStore environmentStore => new EFEnvironmentStore(context);
